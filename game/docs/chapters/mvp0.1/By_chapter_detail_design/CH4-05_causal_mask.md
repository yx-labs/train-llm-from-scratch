# Chapter 4-5 Causal Mask：CausalMask[T,T]
...
CausalMask[T,T] 把「lower triangle」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 CausalMask[T,T] 连接到合约探针。

案例数据输出合约：`float32[T,T]`，示例 shape 为 `[6,6]`。

## 3. 玩家操作

1. 观察预制输入节点和合约探针节点。
2. 添加或修复 `CausalMask[T,T]` 节点。
...
当前任务通过：输出必须保持 dtype=float32，轴为 [T,T]。
