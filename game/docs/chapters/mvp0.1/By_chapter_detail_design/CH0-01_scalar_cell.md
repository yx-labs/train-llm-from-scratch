# Chapter 0-1 Scalar Cell：ScalarCell

## 1. 关卡定位

- 所属章节：Chapter 0 Graph OS 与数字基础
- 构建组件：`ScalarCell`
- 输入来源：Number literal + dtype badge
- 学习目标：一个数字也有类型
- 后续用途：loss、mask value、scale

## 2. 当前案例

ScalarCell 把「Number literal + dtype badge」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 ScalarCell 连接到合约探针。
