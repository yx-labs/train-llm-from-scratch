...
- 构建组件：`AxisTensor`
- 输入来源：Tensor + axis names
- 学习目标：[2,4,8] 不等于 [B,T,C]
- 后续用途：Broadcast / Attention

## 2. 当前案例

AxisTensor 把「Tensor + axis names」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 AxisTensor 连接到合约探针。

案例数据输出合约：`float32[B,T,C]`，示例 shape 为 `[2,4,8]`。

## 3. 玩家操作

1. 观察预制输入节点和合约探针节点。
2. 添加或修复 `AxisTensor` 节点。
