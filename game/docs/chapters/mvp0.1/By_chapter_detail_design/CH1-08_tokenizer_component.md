# Chapter 1-8 Tokenizer Component：TokenizerSocket

## 1. 关卡定位

- 所属章节：Chapter 1 Text → Token Pipeline
- 构建组件：`TokenizerSocket`
- 输入来源：Splitter + Merge + Vocab + Buffer + Mask
- 学习目标：封装成可复用 tokenizer
- 后续用途：所有文本任务

## 2. 当前案例

TokenizerSocket 把「Splitter + Merge + Vocab + Buffer + Mask」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 TokenizerSocket 连接到合约探针。
