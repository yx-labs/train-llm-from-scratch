...
- 构建组件：`EmbeddingTable[V,C]`
- 输入来源：ParameterMatrix
- 学习目标：id 查表得到 vector
- 后续用途：token embedding

## 2. 当前案例

EmbeddingTable[V,C] 把「ParameterMatrix」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 EmbeddingTable[V,C] 连接到合约探针。
