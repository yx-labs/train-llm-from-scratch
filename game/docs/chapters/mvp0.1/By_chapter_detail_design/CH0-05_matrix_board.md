# Chapter 0-5 Matrix Board：Matrix
...
- 输入来源：多条 Vector
- 学习目标：二维结构、row/col
- 后续用途：weight、embedding table

## 2. 当前案例

Matrix 把「多条 Vector」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 Matrix 连接到合约探针。

案例数据输出合约：`float32[C,O]`，示例 shape 为 `[3,2]`。
...
当前任务通过：输出必须保持 dtype=float32，轴为 [C,O]。
