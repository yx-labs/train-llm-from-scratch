...
- 构建组件：`TypedTensor`
- 输入来源：Tensor + dtype
- 学习目标：float/int/bool/mask 不可乱接
- 后续用途：token ids vs hidden

## 2. 当前案例

TypedTensor 把「Tensor + dtype」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 TypedTensor 连接到合约探针。

案例数据输出合约：`int[B,T]`，示例 shape 为 `[1,4]`。

## 3. 玩家操作

1. 观察预制输入节点和合约探针节点。
2. 添加或修复 `TypedTensor` 节点。
