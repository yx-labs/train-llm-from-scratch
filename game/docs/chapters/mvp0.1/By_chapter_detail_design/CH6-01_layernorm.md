# Chapter 6-1 LayerNorm：LayerNorm

## 1. 关卡定位

- 所属章节：Chapter 6 LayerNorm、Residual、Transformer Block
- 构建组件：`LayerNorm`
- 输入来源：normalize + gamma/beta
- 学习目标：稳定每个 token vector
- 后续用途：block

## 2. 当前案例

LayerNorm 把「normalize + gamma/beta」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 LayerNorm 连接到合约探针。
