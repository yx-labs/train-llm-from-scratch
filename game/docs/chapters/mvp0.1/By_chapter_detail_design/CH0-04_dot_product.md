# Chapter 0-4 Dot Product：DotProduct

## 1. 关卡定位

- 所属章节：Chapter 0 Graph OS 与数字基础
- 构建组件：`DotProduct`
- 输入来源：Vector + multiply + sum
- 学习目标：相同长度向量才能点积
- 后续用途：attention score、linear

## 2. 当前案例

DotProduct 把「Vector + multiply + sum」变成可复用的图组件。

任务不是背公式，而是处理一个具体图案例：把准备好的案例数据通过 DotProduct 连接到合约探针。
